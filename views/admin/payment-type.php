...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        ...
        <p class="mt-1 text-sm text-slate-500">Methods of payment accepted from patients.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 p-4 mb-4">
    <div class="flex flex-col sm:flex-row sm:items-end gap-3">
        <div class="flex-1">
            ...
            <div class="relative">
                ...
                <input type="text" id="filterSearch" placeholder="Search type name or description…"
    ...
    <div id="filterSummary" class="hidden mt-3 pt-3 border-t border-slate-100 text-xs text-slate-500">
        Showing <strong id="filteredCount" class="text-slate-900">0</strong> of <strong id="totalCount" class="text-slate-900">0</strong> payment types
    </div>
</div>
...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
    <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    <th class="text-left px-6 py-3.5 font-semibold text-slate-700 text-xs uppercase tracking-wide">Payment Type</th>
            ...
            <tbody class="divide-y divide-slate-100">
                <?php foreach ($paymentTypes as $p): ?>
                    <tr class="hover:bg-slate-50 payment-type-row"
                        data-search="<?= htmlspecialchars(strtolower($p['type_name'] . ' ' . ($p['description'] ?? ''))) ?>">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <p class="font-semibold text-slate-900"><?= htmlspecialchars($p['type_name']) ?></p>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap">
                            <div class="inline-flex items-center gap-1.5">
                                ...
                                <button type="button"
                                        ...
                                        data-payment-type='<?= htmlspecialchars(json_encode($p), ENT_QUOTES, "UTF-8") ?>'>
                                ...
                                <button type="button"
                                        ...
                                        data-payment-type-id="<?= (int)$p['payment_type_id'] ?>"
                                        data-payment-type-name="<?= htmlspecialchars($p['type_name']) ?>">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div id="emptyState" class="hidden text-center py-16">
        ...
        <p class="text-sm font-medium text-slate-700">No payment types match your search</p>
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
        ...
        <div class="flex items-center justify-between px-6 py-5 border-b border-slate-200">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 id="paymentTypeModalTitle" class="text-base font-semibold text-slate-900">New Payment Type</h3>
                    <p class="text-xs text-slate-500">Enter the type name and a short description.</p>
                </div>
            </div>
        ...
        <form id="paymentTypeForm" method="POST" novalidate class="flex-1 overflow-y-auto">
            <input type="hidden" id="payment_type_id" name="payment_type_id">

            <div class="p-6 space-y-5">

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Type Name</label>
                    <input type="text" id="type_name" required placeholder="e.g. Cash, Credit Card, Insurance"
                    ...
                    <p class="mt-1.5 text-xs text-red-600 hidden" data-error-for="type_name"></p>
                </div>
                ...
                <div>
                    ...
                    <textarea id="description" rows="3" placeholder="Briefly describe this payment method"
            ...
            <div class="px-6 py-4 border-t border-slate-200 bg-slate-50 rounded-b-2xl flex items-center justify-end gap-3">
                ...
                <button type="submit" id="paymentTypeSubmitBtn"
                    ...
                    <span id="paymentTypeSubmitLabel">Create Payment Type</span>
                </button>
            </div>
        </form>
    </div>
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div class="p-6">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h3 class="text-base font-semibold text-slate-900">Delete this payment type?</h3>
                    <p class="mt-1.5 text-sm text-slate-500">
                        <strong id="deletePaymentTypeName" class="text-slate-700"></strong> will be permanently removed.
                        This action cannot be undone.
                    </p>
                    <p class="mt-2 text-xs text-slate-500">
                        If any payment is using this type, the delete will be blocked.
                    </p>
                </div>
            </div>
        </div>
