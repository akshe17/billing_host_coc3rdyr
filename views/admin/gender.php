<?php
// views/admin/gender.php
require_once __DIR__ . '/../../controllers/GenderController.php';

$genders = (new GenderController($pdo))->getAll();
$totalGenders = count($genders);
?>

<!-- Page header -->
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Genders</h1>
        <p class="mt-1 text-sm text-slate-500">Gender options used across patient records.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 p-5 mb-6 max-w-xs">
    <p class="text-xs font-medium text-slate-500 uppercase tracking-wide">Total Genders</p>
    <p class="mt-2 text-2xl font-bold text-slate-900"><?= $totalGenders ?></p>
</div>
...
<div class="bg-white rounded-xl border border-slate-200 p-4 mb-4">
    <div class="flex flex-col sm:flex-row sm:items-end gap-3">
        <div class="flex-1">
            ...
            <div class="relative">
                ...
                <input type="text" id="filterSearch" placeholder="Search gender name…"
    ...
    <div id="filterSummary" class="hidden mt-3 pt-3 border-t border-slate-100 text-xs text-slate-500">
        Showing <strong id="filteredCount" class="text-slate-900">0</strong> of <strong id="totalCount" class="text-slate-900">0</strong> genders
    </div>
</div>
...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
    <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    <th class="text-left px-6 py-3.5 font-semibold text-slate-700 text-xs uppercase tracking-wide">Gender</th>
                    <th class="text-right px-6 py-3.5 font-semibold text-slate-700 text-xs uppercase tracking-wide">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                <?php foreach ($genders as $g): ?>
                    <tr class="hover:bg-slate-50 gender-row"
                        data-search="<?= htmlspecialchars(strtolower($g['gender_name'])) ?>">

                        <td class="px-6 py-4">
                            <p class="font-semibold text-slate-900"><?= htmlspecialchars($g['gender_name']) ?></p>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap">
                            <div class="inline-flex items-center gap-1.5">
                                ...
                                <button type="button"
                                        ...
                                        data-gender='<?= htmlspecialchars(json_encode($g), ENT_QUOTES, "UTF-8") ?>'>
                                ...
                                <button type="button"
                                        ...
                                        data-gender-id="<?= (int)$g['gender_id'] ?>"
                                        data-gender-name="<?= htmlspecialchars($g['gender_name']) ?>">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div id="emptyState" class="hidden text-center py-16">
        ...
        <p class="text-sm font-medium text-slate-700">No genders match your search</p>
        <p class="text-xs text-slate-500 mt-1">Try adjusting your search.</p>
    </div>
</div>

<!-- ============ CREATE / EDIT MODAL ============ -->
<div id="genderModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
        ...
        <div class="flex items-center justify-between px-6 py-5 border-b border-slate-200">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 id="genderModalTitle" class="text-base font-semibold text-slate-900">New Gender</h3>
                    <p class="text-xs text-slate-500">Add a gender option for patient records.</p>
                </div>
            </div>
        ...
        <form id="genderForm" method="POST" novalidate class="flex-1 overflow-y-auto">
            <input type="hidden" id="gender_id" name="gender_id">

            <div class="p-6">
                <label class="block text-sm font-medium text-slate-700 mb-1.5">Gender Name</label>
                <input type="text" id="gender_name" required placeholder="e.g. Male, Female, Other"
                ...
                <p class="mt-1.5 text-xs text-red-600 hidden" data-error-for="gender_name"></p>
            </div>
            ...
            <div class="px-6 py-4 border-t border-slate-200 bg-slate-50 rounded-b-2xl flex items-center justify-end gap-3">
                ...
                <button type="submit" id="genderSubmitBtn"
                    ...
                    <span id="genderSubmitLabel">Create Gender</span>
                </button>
            </div>
        </form>
    </div>
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div class="p-6">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h3 class="text-base font-semibold text-slate-900">Delete this gender?</h3>
                    <p class="mt-1.5 text-sm text-slate-500">
                        <strong id="deleteGenderName" class="text-slate-700"></strong> will be permanently removed.
                        This action cannot be undone.
                    </p>
                    <p class="mt-2 text-xs text-slate-500">
                        If any patient is using this gender, the delete will be blocked.
                    </p>
                </div>
            </div>
        </div>
