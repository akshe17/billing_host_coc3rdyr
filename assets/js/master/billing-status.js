...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (openCreate) {
    openCreate.addEventListener("click", () => {
      ...
      document.getElementById("is_paid_status").checked = false;
      modalTitle.textContent = "New Billing Status";
  ...
  document.querySelectorAll(".edit-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      modalTitle.textContent = "Edit Billing Status";
      ...
      document.getElementById("is_paid_status").checked =
        parseInt(s.is_paid_status, 10) === 1;
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      is_paid_status: document.getElementById("is_paid_status").checked ? 1 : 0,
    };

    const url = isEdit
      ? `${baseUrl}/api/billing-statuses/update.php?id=${id}`
      : `${baseUrl}/api/billing-statuses/create.php`;
    ...
    confirmDeleteBtn.addEventListener("click", async () => {
      ...
      try {
        const { data } = await axios.post(
          `${baseUrl}/api/billing-statuses/delete.php?id=${pendingDeleteId}`,
          {},
          {
            headers: { "Content-Type": "application/json" },
            withCredentials: true,
  ...
  const rows = Array.from(document.querySelectorAll(".billing-status-row"));
